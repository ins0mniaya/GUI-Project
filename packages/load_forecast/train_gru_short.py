# -*- coding: utf-8 -*-
# 用电负荷短期 GRU 训练文件
# 功能：读取并清洗负荷数据，训练“历史24h -> 未来4h”的多输出 GRU。
...
DEFAULT_SHORT_METRICS_PATH = DEFAULT_SHORT_MODEL_PATH.with_suffix(".metrics.json")
DEFAULT_SHORT_TEST_CSV_PATH = RESULTS_DIR / "load_gru_short_4h_test_predictions.csv"
DEFAULT_SHORT_PLOT_PATH = RESULTS_DIR / "load_gru_short_4h_prediction_vs_true.png"
DEFAULT_SHORT_HORIZON_PLOT_PATH = RESULTS_DIR / "load_gru_short_4h_horizon_mae.png"
...
def _extract_max_history_lag(feature_cols: list[str]) -> int:
    ...
    for col in feature_cols:
        if col.startswith(("load_lag_", "load_ma_", "load_diff_")):
...
def _make_windows(values: np.ndarray, load_col_idx: int, input_len: int, output_len: int):
    """把连续时序样本切成：历史 input_len 小时 -> 未来 output_len 小时。"""
    xs, ys, starts = [], [], []
    max_start = len(values) - input_len - output_len + 1
    for start in range(max_start):
        xs.append(values[start:start + input_len])
        ys.append(values[start + input_len:start + input_len + output_len, load_col_idx])
        starts.append(start)
    return np.asarray(xs, dtype=np.float32), np.asarray(ys, dtype=np.float32), np.asarray(starts, dtype=np.int64)
...
def _horizon_metrics(y_true: np.ndarray, y_pred: np.ndarray) -> list[dict]:
    """分别统计每个预测步长上的误差。"""
    return [{"horizon": idx + 1, **_metrics(y_true[:, idx], y_pred[:, idx])} for idx in range(y_true.shape[1])]
...
def _plot_sample_prediction(
    ...
    fig, ax = plt_obj.subplots(figsize=(9.5, 4.8))
...
def train_gru_short(
    ...
    output_path: str | Path = DEFAULT_SHORT_MODEL_PATH,
    input_len: int = DEFAULT_INPUT_LEN,
    output_len: int = SHORT_FORECAST_HOURS,
    epochs: int = 45,
    batch_size: int = 256,
    lr: float = 8e-4,
    patience: int = 8,
    metrics_out: str | Path = DEFAULT_SHORT_METRICS_PATH,
    test_csv_out: str | Path = DEFAULT_SHORT_TEST_CSV_PATH,
    plot_out: str | Path = DEFAULT_SHORT_PLOT_PATH,
    horizon_plot_out: str | Path = DEFAULT_SHORT_HORIZON_PLOT_PATH,
) -> dict:
    """训练短期 4h GRU，并像光伏训练文件一样同步输出 pth、CSV、JSON 和图片。"""
    ...
    x_all, y_all, sample_starts = _make_windows(scaled_values, load_col_idx, input_len, output_len)
    ...
    print(f"Input length: {input_len}h, output length: {output_len}h")
    ...
    for epoch in range(1, epochs + 1):
        ...
        model.eval()
        with torch.no_grad():
            val_pred = model(torch.tensor(x_val, dtype=torch.float32).to(device)).cpu()
    ...
    model.eval()
    with torch.no_grad():
        test_pred_scaled = model(torch.tensor(x_test, dtype=torch.float32).to(device)).cpu().numpy()
    ...
    horizon_plot_path = _plot_horizon_mae(
        horizon_metrics,
        horizon_plot_out,
        "短期4h GRU测试集各预测步长 MAE",
    )

    checkpoint = {
        "model_type": "gru_direct_4h",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="训练短期 4h GRU 负荷预测模型，并同步输出 pth、CSV、JSON 和图片。")
    parser.add_argument("--data-path", default=str(LOAD_DATA_PATH))
    parser.add_argument("--output-path", default=str(DEFAULT_SHORT_MODEL_PATH))
    parser.add_argument("--input-len", type=int, default=DEFAULT_INPUT_LEN)
    parser.add_argument("--output-len", type=int, default=SHORT_FORECAST_HOURS)
    parser.add_argument("--epochs", type=int, default=45)
    parser.add_argument("--batch-size", type=int, default=256)
    parser.add_argument("--lr", type=float, default=8e-4)
    parser.add_argument("--patience", type=int, default=8)
    parser.add_argument("--metrics-out", default=str(DEFAULT_SHORT_METRICS_PATH))
    parser.add_argument("--test-csv-out", default=str(DEFAULT_SHORT_TEST_CSV_PATH))
    parser.add_argument("--plot-out", default=str(DEFAULT_SHORT_PLOT_PATH))
    parser.add_argument("--horizon-plot-out", default=str(DEFAULT_SHORT_HORIZON_PLOT_PATH))
